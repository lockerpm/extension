...
class AutofillMenuIframeService {
  ...
  private readonly windowMessageHandlers = {
    updateAutoSaveBarHeight: (message: any) => this.updateElementStyles(this.iframe, message.styles),
  };
  private readonly backgroundPortMessageHandlers = {
    initAutoSaveBar: ({ message }) => this.initAutoSaveBar(message),
  ...
  private initAutoSaveBar(message: any) {
    if (message?.initData) {
  ...
  private handleElementAttributeMutation(element: HTMLElement) {
    ...
    for (let attributeIndex = 0; attributeIndex < attributes.length; attributeIndex++) {
      ...
      if (this.foreignMutationsCount >= 10) {
        this.port?.postMessage({ command: "forceCloseAutoSaveBar" });
        break;
      }
