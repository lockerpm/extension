...
import {
  ...
  AutofillMenuIframeService as AutofillMenuIframeServiceInterface,
  AutofillMenuIframeExtensionMessage,
  AutofillMenuIframeWindowMessageHandlers,
} from "./abstractions/autofill-menu-iframe.service";

class AutofillMenuIframeService implements AutofillMenuIframeServiceInterface {
  ...
  private mutationObserverIterationsResetTimeout: any;
  private readonly windowMessageHandlers: AutofillMenuIframeWindowMessageHandlers = {
    updateAutofillMenuListHeight: (message) => this.updateElementStyles(this.iframe, message.styles),
    getPageColorScheme: () => this.updateMenuPageColorScheme(),
  };
  private readonly backgroundPortMessageHandlers: BackgroundPortMessageHandlers = {
    initAutofillMenuList: ({ message }) => this.initAutofillMenuList(message),
    updateIframePosition: ({ message }) => this.updateIframePosition(message.styles),
    updateMenuHidden: ({ message }) => this.updateElementStyles(this.iframe, message.styles),
  };
   ...
   * does not exist.
   */
  private updateMenuPageColorScheme() {
    const colorSchemeValue = globalThis.document
      .querySelector("meta[name='color-scheme']")
      ?.getAttribute("content");

    this.iframe.contentWindow?.postMessage(
      { command: "updateMenuPageColorScheme", colorScheme: colorSchemeValue || "normal" },
      "*",
    );
  }
  ...
  private handleElementAttributeMutation(element: HTMLElement) {
    ...
    for (let attributeIndex = 0; attributeIndex < attributes.length; attributeIndex++) {
      ...
      if (this.foreignMutationsCount >= 10) {
        this.port?.postMessage({ command: "forceCloseAutofillMenu" });
        break;
      }
...
export default AutofillMenuIframeService;
